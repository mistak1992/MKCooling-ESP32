...
#define MKC_PERSIST_MODULE_FILENAME "mkc_persist"

static bool is_init = false;
char *head = "/spiffs/";


char* mkc_int2String(int num, char *str)//10进制 
{
	int i = 0;//指示填充str 
...
int mkc_string2Int(char *str)//字符串转数字 
{
	char flag = '+';//指示结果是否带符号 
	long res = 0;
...
mkc_persist_model_t persist_read_file(char *file_name){
    // Open renamed file for reading
    ESP_LOGI(MODULE_TAG, "Reading file");
    mkc_persist_model_t model;
    char *tail = file_name;
...
void persist_write_file(char *file_name, mkc_persist_model_t model){
    // First create a file.
    char *tail = file_name;
    char *name = (char *)calloc(strlen(head) + strlen(tail) + 2, sizeof(char));
    strcpy(name, head);
...
// void mkc_persist_set_data(enum mkc_persist_module_type type, uint16_t value){
//     if (is_init == false)
//     {
//         mkc_persist_module_init();
//     }
//     switch (type){
//     case MKC_PERSIST_MODULE_DUTY:{
//         char value_str[8] = {};
//         persist_write_file("fan_duty", mkc_int2String(value, value_str));
//         break;
//     }
//     case MKC_PERSIST_MODULE_MLX90614_SWITCH:{
//         char value_str[8] = {};
//         persist_write_file("MLX90614_SWITCH", mkc_int2String(value, value_str));
//         break;
//     }
//     case MKC_PERSIST_MODULE_AUTH:{
//         char value_str[8] = {};
//         persist_write_file("AUTH", mkc_int2String(value, value_str));
//         break;
//     }
//     default:
//         break;
...
// char* mkc_persist_get_data(enum mkc_persist_module_type type, char *value){
//     if (is_init == false)
//     {
//         mkc_persist_module_init();
//     }
...
void mkc_persist_set_data(mkc_protocol_model_t data){
    ...
    model.temp_ir_a_data = data.temp_ir_a_data;
    model.temp_ir_o_data = data.temp_ir_o_data;
    model.fan_duty = data.fan_duty;
    model.fan_rpm = data.fan_rpm;
    model.delay = data.delay;
    persist_write_file(MKC_PERSIST_MODULE_FILENAME, model);
    // switch (type){
    // case MKC_PERSIST_MODULE_DUTY:{
    //     char value_str[8] = {};
    //     persist_write_file("fan_duty", mkc_int2String(value, value_str));
...
mkc_protocol_model_t mkc_persist_get_data(){
    ...
    memset(&model, 0, sizeof(model));
    mkc_persist_model_t data = persist_read_file(MKC_PERSIST_MODULE_FILENAME);
    model.temp_ir_a_data = data.temp_ir_a_data;
    model.temp_ir_o_data = data.temp_ir_o_data;
    model.fan_duty = data.fan_duty;
    model.fan_rpm = data.fan_rpm;
